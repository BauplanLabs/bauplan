...
use crate::{ApiRequest, ApiResponse, Profile, grpc, python::exceptions::BauplanError};
...
mod bauplan {
    ...
    impl Client {
        pub(crate) fn roundtrip<T: ApiRequest>(&self, req: T) -> PyResult<T::Response> {
            let req = req
                .into_request(&self.profile)
            ...
            let resp =
                <T::Response as ApiResponse>::from_response(resp.map(ureq::Body::into_reader))?;
            Ok(resp)
        }
    }

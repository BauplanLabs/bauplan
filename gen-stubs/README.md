...
All stubs live at the public path `python/bauplan/`. This ensures tools like
griffe resolve types through `bauplan` rather than `bauplan._internal`. The
`__init__.pyi` stub is generated by prepending `__init__.py` (with
`_internal` imports stripped) to the auto-generated class definitions, so that
type checkers see both the pure-Python re-exports and the extension types.
Imports from `bauplan._internal` are whitelisted in `pyproject.toml` via
`allowed-unresolved-imports`, since `_internal` is a compiled extension module
without stubs.

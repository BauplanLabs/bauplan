...
class TableSchema:
    ...
    References to schema columns can be made by using "index syntax" and passing it to
    the `lineage` parameter of `TableField`, for example:
    ...
    class NewTableSchema(TableSchema):
        first: Annotated[Int64, TableField(name='first col', doc='first column')]

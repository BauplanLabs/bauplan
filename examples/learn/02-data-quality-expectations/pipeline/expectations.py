...
# An expectation only needs the columns it validates, so we declare them as a schema and
# let Bauplan push the projection down to the scan.
class OnSceneDatetime(TableSchema):
    """The column this expectation checks for completeness."""
...
def test_null_values_on_scene_datetime(
    data: Annotated[
        ...
        Model(
            ...
            projection_schema=OnSceneDatetime,
        ),
    ],
) -> bool:

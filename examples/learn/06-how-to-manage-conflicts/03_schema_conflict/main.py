...
def main(
    ...
    In order:
      1. The old pipeline runs on the branch. It computes the mean Titanic fare per
         ...
         decimal(4, 2) so the money is exact rather than binary floating point. The
         output schema leaves the decimal field as `Any`, while the expectation checks
         that the fares being averaged fit that precision. The run succeeds.
      ...
      3. The pipeline is modified and the filter on `Pclass` is dropped, so first
         ...
         for first class women is 106.13, so neither fits decimal(4, 2).
      4. The expectation fires on the widened input and fails the run, so no drift
         ...
         decimal(5, 2) in the model and raise the bound the expectation checks. The
         schema remains `Any` because decimal types are not pinned by the typed SDK.
    """
    ...
    print(
        "\n=== Step 1: ship the old pipeline "
        "(mean standard fare per Pclass, Fare: decimal(4, 2)) ===\n"
    )

    # Strict mode is enabled by default; setting it explicitly ensures failing
    # expectations fail the run before tables that fail validation can be merged
    run_state = client.run(
        project_dir="pipelines/old_pipeline", ref=branch, strict=True
    )
    ...
    # Fare as decimal(4, 2) and still carries the expectation that fares must fit
    ...
    run_state = client.run(
        project_dir="pipelines/new_pipeline", ref=branch, strict=True
    )

...
| 5. **Run and validate** | Run `uv run bauplan run --dry-run` against the project to validate model outputs and execute the expectations from step 4 without materializing tables. Strict mode is enabled by default, so failing expectations fail the run. |

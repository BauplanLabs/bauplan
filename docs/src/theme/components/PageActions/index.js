...
import { Check, ChevronDown, CircleAlert, Copy, ExternalLink, FileText } from "lucide-react";
...
export default function PageActions() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setCopyStatus("idle"), copyStatus === "failed" ? 4000 : 2000);
  ...
  return (
    <div ref={container} className="relative flex-shrink-0">
      ...
      {copyStatus === "failed" && (
        <div
          role="alert"
          className="fixed bottom-4 right-4 z-[var(--ifm-z-index-overlay)] flex items-center gap-2 px-3 py-2 text-sm rounded-md shadow-md border border-solid border-[var(--ifm-color-emphasis-300)] bg-[var(--ifm-background-surface-color)] text-[var(--ifm-font-color-base)]"
        >
          <CircleAlert className="h-4 w-4 text-[var(--ifm-color-danger)]" />
          Couldn't copy this page.
        </div>
      )}
    </div>
  );

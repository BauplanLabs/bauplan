...
requires the name of the inputs with the desired columns and, optionally,
filters.
...
`filter` can only be applied to an input that is read directly from the catalog.
If the input is another model in the same pipeline, the DAG is rejected.

In `my_model` the filter is valid: `input_table` is not defined anywhere in the
pipeline, so it must be a table in the catalog.
...
def my_model(
    data: Annotated[
        pyarrow.Table,
        bauplan.Model('input_table', filter="timestamp <= '2022-12-16'")
    ]
) -> pyarrow.Table:
    return data
```

In `other_model` it is not: `my_model` is a node of the same DAG, not a catalog
table.
...
def other_model(
    data: Annotated[
        pyarrow.Table,
        # rejected: my_model is a pipeline node, not a catalog table
...
To get the same result, filter inside the body of `my_model` before returing it, or filter `data`
at the top of `other_model`.
...
defined by the interpreter version and dependencies you declare in
...
- The filter also supports the use of [parameters](/common-scenarios/parameterized-runs).
...
a single, pure SQL query executed by the Bauplan query engine. Inside the Python
function of a **Python model**, SQL can be executed using an embedded SQL engine of your
choice such as DuckDB.
...
The query lives in `my_model.sql`. The `output_schema` header ties the query to
that schema, the way the return annotation does in a Python model.
...
By default, Bauplan discovers `.sql` files under the project root and derives the
model name from the filename, so this query defines a model named `my_model`.
Downstream models reference it like any other node, with `bauplan.Model('my_model')`.

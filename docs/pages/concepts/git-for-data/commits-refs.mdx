...
Bauplan versions **everything**: code, data, and execution
environments.

- **Code** lives in Git or similar tools, and Bauplan also captures it
  immutably at execution time.
- **Data** changes through the catalog: evolving table snapshots track
  all writes, replacements, and deletions, where each
  data change creates a new commit.
- **Execution environments** use code-based definitions and versioning as
...
- In the case of a merge operation, a second parent hash,
  `parent_hash[1]`, that represents the merged branch.

...
def test_critical_field_completeness(
    data: Annotated[
        pyarrow.Table,
        bauplan.Model('my_model', projection_schema=CriticalField),
    ],
) -> bool:
...
def test_data_freshness(
    data: Annotated[pyarrow.Table, bauplan.Model('model', projection_schema=Timestamps)],
) -> bool:
...
By default, Bauplan enforces expectations: if any expectation fails, the pipeline halts immediately.

When failures are not critical, you can log them and let the pipeline continue by disabling strict mode on the `run` command: pass `--no-strict` in the [CLI](/reference/cli#bauplan-run) or `strict=False` in the [Python SDK](/reference/bauplan#bauplan-client-run).

...
Post-hoc validation frameworks, like [dbt tests](https://docs.getdbt.com/docs/build/data-tests)
 or [Great Expectations](https://greatexpectations.io/), commonly execute validation as separate queries after a table or view is built. Issues are detected only after the object exists, which means invalid data may already be visible to downstream consumers, and jobs used to build the object must be rerun.

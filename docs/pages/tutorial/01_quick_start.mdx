...
- **`bauplan_project.yaml`** - project metadata (id and name), this is also the place where [secrets](https://github.com/BauplanLabs/examples/tree/main/learn/04-using-llms-with-secrets) and [parameters](/reference/cli#parameter) are stored.
...
The scaffolded project defines a small pipeline: it reads from the `titanic` table, computes survival rates by age, and validates the output with an expectation test. Let's run it:
...
```bash
bauplan table ls | grep survival_rate_by_age
```

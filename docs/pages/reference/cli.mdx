...
| `--nocache` | - | string | - | Disable caching |
...
| `--no-transaction` | `-t` | string | - | Disable transactional branching. Without this flag, the server will create a temporary branch where models are materialized. Once all models succeed, it will be merged to branch in which this run is happening in |
...
| `--no-cache` | - | string | - | Disable caching |

...
5. **Re-run transactionally in main** - The pipeline re-executes against production data. [Data expectations](/concepts/expectations) run again in main before anything is promoted. Note that you can still run on a transactional branch here etc., but the key observation is that since you promoted the code, and the data is a byproduct of the code in Bauplan, you are re-generating the data assets with the very last available source inputs.

...
description: "Fix schema conflicts when creating Bauplan tables from S3 files. Generate a table plan, cast conflicting column types in YAML or Python, then apply and import data."
---
...
<Note>
  ...
  import the data later via [`table
  import`](/reference/cli#bauplan-table-import), using the same `--search-uri`
  or a different one with the same schema.
</Note>

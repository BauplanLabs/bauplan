...
When you're passing in a dataset via `--search-uri` option in [`table create`](/reference/cli#table-create) command, Bauplan scans your files to infer the table
schema. If your files have **inconsistent columns or types**, the create
will fail with a schema conflict.

Under the hood, the [`table create`](/reference/cli#table-create)
...
<Note>
  ...
  import the data later via [`table import`](/reference/cli#table-import), using

...
<Note>
  A judicious use of the `columns` parameter in the `@bauplan.model` decorator
  can complement expectations and further enforce schema compliance by
  restricting the columns the function will output. For instance, the following
  would fail when `strict="on"`. For other uses of expectations in Bauplan,
...
def give_back_age(data=bauplan.Model("titanic")):

...
[Expectations](/concepts/expectations) turn that implicit contract into executable code that lives next to the model. Strict mode is enabled by default, so a failed expectation fails the run. The example sets `strict=True` explicitly:
...
<Note>
  ...
  mismatch: the run fails with a `bauplan.ModelOutputContractError` and
  nothing is materialized. For other uses of expectations in Bauplan,

...
- **Production / scheduled pipelines** - a scheduler (for example Airflow, cron) triggers a pipeline on a recurring cadence and does not need to hold a connection open for the entire run.
...
Use [`job get`](/reference/cli#job-get) to check a single job:
...
Use [`job ls`](/reference/cli#job-ls--job-list) to see recent jobs:

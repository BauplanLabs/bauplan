...
Strict mode is enabled by default: runtime warnings, such as column mismatches or failing [expectations](/concepts/expectations), fail the run. To allow the run to complete with warnings, pass `--no-strict`:
